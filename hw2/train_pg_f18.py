...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope):
        ...
        output_placeholder = tf.layers.dense(hidden, output_size, activation=activation)
...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            # YOUR_CODE_HERE
            logits = build_mlp(sy_ob_no, self.ac_dim, 'discrete', self.n_layers, self.size)
            sy_logits_na = tf.nn.log_softmax(logits)
            return sy_logits_na
        else:
            # YOUR_CODE_HERE
            sy_mean = build_mlp(sy_ob_no, self.ac_dim, 'continuous', self.n_layers, self.size)
            sy_logstd = tf.get_variable("logstd", [self.ac_dim], initializer=tf.zeros_initializer())
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            sy_logits_na = policy_parameters
            # YOUR_CODE_HERE
            one_hot = tf.one_hot(sy_ac_na, self.ac_dim)
            sy_logprob_n = tf.reduce_sum(tf.multiply(one_hot, sy_logits_na), 1)
        else:
            sy_mean, sy_logstd = policy_parameters
            # YOUR_CODE_HERE
            diff = sy_ac_na - sy_mean
            sy_logprob_n = - 0.5 * tf.reduce_sum(diff ** 2 / (tf.exp(sy_logstd) ** 2), axis=1) \
                           - self.ac_dim * 0.5 * tf.log(tf.constant(2 * np.pi)) \
    ...
    def build_computation_graph(self):
        ...
        #========================================================================================#
        self.loss = -tf.reduce_sum(self.sy_adv_n * self.sy_logprob_n)
    ...
    def sum_of_rewards(self, re_n):
        ...
        if self.reward_to_go:
            for path in re_n:
                temp = scipy.signal.lfilter([1], [1, -self.gamma], path[::-1], axis=0)[::-1]
                for t in temp:
                    q_n.append(t)
        else:
            for path in re_n:
    ...
    def compute_advantage(self, ob_no, q_n):
        ...
        if self.nn_baseline:
            ...
            b_n = self.sess.run(self.baseline_prediction, feed_dict={self.sy_ob_no : ob_no[0][None]})
            adv_n = q_n - b_n
        else:
            adv_n = q_n.copy()
        return adv_n
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        if self.nn_baseline:
            ...
            # YOUR_CODE_HERE
            target_n, _loss = self.sess.run([self.baseline_update_op, self.baseline_loss], feed_dict={self.sy_ob_no:ob_no, self.sy_target_n:q_n})
            print('Hi', _loss)
        ...
        # YOUR_CODE_HERE
        _, l = self.sess.run([self.update_op, self.loss], feed_dict={self.sy_ob_no: ob_no,
                                                                     self.sy_ac_na: ac_na,
                                                                     self.sy_adv_n: adv_n})
        print(l)
